...
tags:
  ...
  - パフォーマンス
  - ドキュメント翻訳
description: "The Go compiler のドキュメントを日本語に翻訳。コンパイラのフェーズごとの役割、型チェック、SSA 形式への変換、最適化からコード生成までの流れを詳しく解説します。"
---
...
- [`bent`](https://pkg.go.dev/golang.org/x/benchmarks/cmd/bent) は、Docker コンテナ内で様々なコミュニティの Go プロジェクトからの大規模なベンチマークセットを実行するのを支援します。
